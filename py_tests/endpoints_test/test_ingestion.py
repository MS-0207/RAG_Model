...
def test_ingest_endpoint(mock_delay):
    ...
    assert response.json()["message"] == (
        "Document ingestion started in the background."
    )
...
def test_run_ingestion_pipeline(
    ...
    mock_load_all_documents.assert_called_once_with(settings.raw_dir)

    mock_chunk_documents.assert_called_once_with(documents)
...
#     # from api.exception import NoDocumentsFoundError
#
#     mock_run_ingestion_pipeline.side_effect = NoDocumentsFoundError(
#         "No supported documents found."
#     )
#
#     response = client.post("/documents/ingest")
...
#     assert response.json()["error"] == ("No supported documents found.")
